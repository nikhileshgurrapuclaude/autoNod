...
// ---------------------------------------------------------------------------
const DEFAULTS = {
  allowedDirs: ['src/', 'app/', 'pages/', 'components/', 'styles/', 'public/', 'assets/', 'lib/'],
...
const BASH_SAFE_PATTERNS = [
  /^\s*(npm|pnpm|yarn|bun)\s+(run\s+)?(dev|build|start|lint|test|format|typecheck|check)\b/i,
  /^\s*(npx\s+)?(tsc|eslint|prettier|vite|next|svelte-kit|astro)\b/i,
  /^\s*git\s+(status|diff|log|branch|show)\b/i,
  /^\s*(ls|pwd|cat\s+package\.json|echo)\b/i,
];
...
function isAllowedFrontendFile(rel) {
  ...
  return config.allowedDirs.some((d) => rel.startsWith(d) || rel.includes('/' + d));
}
...
if (toolName === 'WebFetch' || toolName === 'WebSearch') {
  if (config.denyAllNetworkTools) {
    decide('deny', `${toolName} blocked — no network egress from Claude (prevents leaking secrets/backend info).`);
  }
  fallThrough();
}
...
if (toolName === 'Bash') {
  ...
  if (dom) decide('deny', `command references protected backend host "${dom}".`);
  if (matchesAny(cmd, BASH_DENY_PATTERNS)) {
    decide('deny', 'command reads secrets or reaches the network (env dump / curl / wget / ssh / scp, etc.).');
  }
  if (config.autoApproveSafeBash && matchesAny(cmd, BASH_SAFE_PATTERNS)) {
    decide('allow', 'recognized safe frontend command.');
  }
  fallThrough(); // unknown bash still prompts — deliberately conservative
}
...
if (['Write', 'Edit', 'MultiEdit', 'NotebookEdit'].includes(toolName)) {
  ...
  if (matchesAny(blob, SECRET_CONTENT_PATTERNS)) {
    decide('deny', 'the content being written looks like a secret (private key / token / connection string).');
  }
  const dom = referencesBackendDomain(blob);
  if (dom) decide('deny', `the content being written references protected backend host "${dom}".`);
}

// Allow safe frontend file edits.
if (['Write', 'Edit', 'MultiEdit', 'NotebookEdit', 'Read'].includes(toolName) && paths.length > 0) {
